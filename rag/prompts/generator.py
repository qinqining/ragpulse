...
CHUNK_ENHANCED_SUMMARY_TASK_ZH = """你的任务：
对输入的文档片段生成一份「增强检索摘要（可直接拿来做向量检索）」。
要求：
- 提取片段中的关键事实、数字与数据要点（如有）
- 概括主要主题与核心概念（如有）
- 提炼片段能够回答的典型问题（用一句话表达即可）
- 若片段包含表格/图表的文字描述，也要提取其结论或可检索要点
- 给出用户可能用来检索的替代搜索词（同义词/关键词变体）

请确保摘要详细且便于检索，且“优先可查找性而非简洁性”。
直接输出摘要正文，不要寒暄，不要分点标题以外的多余套话。"""


def chunk_enhanced_summary_prompt() -> str:
    """生成“chunk 增强摘要”的用户侧提示词。"""
    return CHUNK_ENHANCED_SUMMARY_TASK_ZH + "\n\n【当前输入】文档片段："
