...
except ImportError:  # 可选：仅 PDF/简历等编号解析需要
    class _RomanStub:
        @staticmethod
        def number(_s):
            raise ValueError("roman_numbers not installed")
...
try:
    ...
    class _W2nStub:
        @staticmethod
        def word_to_num(_s):
            raise ValueError("word2number not installed")
...
try:
    ...
    def cn2an(_s, **kwargs):
        raise ValueError("cn2an not installed")
...
def find_codec(blob):
    if chardet is not None:
        ...
        if detected.get("confidence", 0) > 0.5:
            if detected.get("encoding") == "ascii":
                return "utf-8"
